...
const PlanBilling = ({ onSubscriptionActivated }) => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      const response = await fetch(apiConfig.endpoints.pricing.fetchProductsAndPricing(), {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          user_id: user.uid,
          training_data: null // Add any training data if needed
        })
      });
  ...
  const getStatusColor = (status) => {
    ...
    if (s === 'coming soon') return 'bg-gray-200 text-gray-700';
    if (s === 'available') return 'bg-blue-100 text-blue-800';
  ...
  const getStatusIcon = (status) => {
    const s = (status || '').toLowerCase();
    if (s === 'active') return '✅';
    if (s === 'inactive') return '⏸️';
    if (s === 'coming soon') return '🛠️';
    if (s === 'available') return '🟢';
    return '❓';
  };

  const convertCurrency = (inrAmount) => {
    // Approximate exchange rates (you might want to fetch these from an API)
    const usdRate = 0.012; // 1 INR = 0.012 USD
    const eurRate = 0.011; // 1 INR = 0.011 EUR
    
    return {
      inr: inrAmount,
      usd: (inrAmount * usdRate).toFixed(2),
      eur: (inrAmount * eurRate).toFixed(2)
    };
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">View and manage your subscription plans and billing details</p>
        </div>
        <button
      ...
      {!loading && !error && products.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-600">No subscription products are currently available.</p>
        </div>
      )}
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                      isComingSoon ? 'bg-gray-200' : 'bg-blue-100'
                    }`}>
                  ...
                  <span className={`px-3 py-1 text-xs font-medium rounded-full flex items-center space-x-1 ${getStatusColor(product.status)}`}>
                ...
                <div className="space-y-3">
                  {product.base_price ? (
                    ...
                    <div className={`rounded-lg p-3 ${isComingSoon ? 'bg-gray-100' : 'bg-gray-50'}`}>
                      <p className={`text-sm ${isComingSoon ? 'text-gray-500' : 'text-gray-600'}`}>Pricing</p>
                      <p className={`text-lg font-semibold ${isComingSoon ? 'text-gray-500' : 'text-gray-500'}`}>Not Available</p>
                    </div>
                  )}
                </div>
