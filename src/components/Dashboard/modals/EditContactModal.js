...
const EditContactModal = ({ isOpen, contact, onClose, onSuccess }) => {
  ...
  // Parse phone number to extract country code and dial code
  const parsePhoneNumber = (phoneNumber) => {
    ...
    if (phoneNumber && phoneNumber.startsWith('+')) {
      ...
      // Check for longer dial codes first
  ...
  useEffect(() => {
    if (isOpen && contact) {
      ...
      const parsedPhones = phones.length > 0 ? phones.map(phoneObj => {
        const parsed = parsePhoneNumber(phoneObj.phone || '');
        return {
          phone: parsed.phone,
      ...
      setContactLeadStage(contactData.lead_stage || 'NEW');
      setContactError('');
      setContactSuccess('');
    }
  }, [isOpen, contact]);
